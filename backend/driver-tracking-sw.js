...
const CACHE_NAME   = 'cmp-driver-v12';
// Never cache the HTML — always fetch fresh so UI updates deploy instantly
const ASSETS_CACHE = [
  'config.js',
...
// ── Fetch: network-first for HTML, API calls, cache-first for static assets ──
self.addEventListener('fetch', function (evt) {
  var url = evt.request.url;

  // Always network-first for non-GET
  ...
  // Always network-first for HTML — never serve stale cached page
  if (url.includes('driver-tracking.html') || evt.request.headers.get('accept') === 'text/html') {
    evt.respondWith(
      fetch(evt.request).then(function (response) {
        ...
        return caches.match(evt.request); // offline fallback only
      })
    );
    return;
  }

  // Cache-first for known static assets (leaflet etc), network-update in background
  evt.respondWith(
